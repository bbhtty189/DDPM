        ...
        for i in range(levels):
            ...
            for _ in range(n_res_blocks):
                ...
                if i in attention_levels:
                    layers.append(SpatialTransformer(channels, n_heads, tf_layers, channels))
...
if __name__ == '__main__':
    ...
    model = UNetModel(in_channels=3, out_channels=3, channels=128, n_res_blocks=2, attention_levels=[4], channel_multipliers=[1, 1, 2, 2, 4, 4], n_heads=8, d_cond=32*32)
    model = model.to('cuda:0')
    from torchsummary import summary
    x = torch.randint(0, 10, (1, 3, 32, 32)).float().to('cuda:0')
    t = torch.randint(0, 1000, (1, ), device='cuda:0')
    cond = None
